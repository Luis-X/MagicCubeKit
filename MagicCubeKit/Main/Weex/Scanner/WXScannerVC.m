...
- (BOOL)remoteDebug:(NSURL *)url
{
    ...
    for (NSString *param in [query componentsSeparatedByString:@"&"]) {
        ...
        if ([[elts firstObject] isEqualToString:@"_wx_debug"]) {
            ...
            if ([[[self.navigationController viewControllers] objectAtIndex:0] isKindOfClass:NSClassFromString(@"WeexViewController")]) {
                WeexViewController * vc = (WeexViewController*)[[self.navigationController viewControllers] objectAtIndex:0];
